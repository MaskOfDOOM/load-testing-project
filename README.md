...
> **Примечание:** По умолчанию CSV отключён для экономии места. При необходимости включите в `config.yaml`: `results.generate_csv: true`.
...
|-----|-------------|------------|
| **Stages** | Частично | Studio может сгенерировать `iterations: 1` — заменить на `stages:` из `basic-web-test.js` |
| **Batch-запросы** | Да | Studio генерирует `http.batch()` 
| **Custom metrics** | Нет | Studio не генерирует `Rate` и `Trend`.
